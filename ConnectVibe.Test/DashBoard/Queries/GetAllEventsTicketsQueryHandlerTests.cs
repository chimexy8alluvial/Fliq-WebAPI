...
public class GetAllEventsTicketsQueryHandlerTests
{
    ...
    public async Task Handle_WithSoldOutFilter_ReturnsFilteredResults()
    {
        ...
        var query = new GetAllEventsTicketsQuery(paginationRequest, "Music", "SoldOut", startDate, endDate, "New York");
    ...
    public async Task Handle_WithCancelledFilter_ReturnsFilteredResults()
    {
        // Arrange
        var paginationRequest = new PaginationRequest(1, 10);
        var query = new GetAllEventsTicketsQuery(paginationRequest, null, "Cancelled");
    ...
    public async Task Handle_RepositoryThrowsException_ReturnsError()
    {
        ...
        _loggerMock!.Setup(x => x.LogError($"Error fetching events with tickets: {exception.Message}"));
